...
Spec: [`SPEC.md`](SPEC.md). Lint: `python3 lint.py <path>` (or `./scripts/lint-gmd.sh`).
...
Validate before committing: `python3 lint.py <path>` — zero errors. The full
conformance spec is [`SPEC.md`](SPEC.md).
...
| Command | Purpose |
|---------|---------|
| `./gmd lint <path>` | Validate GMD docs (exit 1 on errors) |
| `./gmd init <root>` | Scaffold a project for GMD authoring |
| `./gmd slice <file> "<query>"` | Retrieve a relevant subset of a GMD doc |
| `./scripts/lint-gmd.sh` | Lint this repo's own docs (CI entry point) |
